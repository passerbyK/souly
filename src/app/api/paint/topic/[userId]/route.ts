...
export async function GET(
  ...
  try {
    ...
    if (!user) {
      return NextResponse.json({ error: "Couldn't Get Any Topics: No User" }, { status: 404 });
    }

    const createdTime = subject?.createdAt.setHours(0, 0, 0, 0); // default 00:00
    if (!createdTime) {
      return NextResponse.json({ error: "Couldn't Get Any Topics: Subject Created Time" }, { status: 404 });
    }
    ...
    if (dayIndex > user?.lastingDays) {
      return NextResponse.json({ error: "Couldn't Get Any Topics: No Topics Now" }, { status: 404 });
    }

    const topic = subject?.topic ?? "[]";
    const topicArray = JSON.parse(topic);
    if (!topicArray) {
      return NextResponse.json({ error: "Couldn't Get Any Topics: Topic Lists" }, { status: 404 });
    }
    
    const targetElement: string = topicArray[dayIndex];
    if (!targetElement) {
      return NextResponse.json({ error: "Couldn't Get Any Topics: Target Element" }, { status: 404 });
    }
